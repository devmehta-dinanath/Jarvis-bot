...
_MEETING_TEXT_SIGNALS = re.compile(
    ...
    r"share screen|participants?\s*\()",
    re.IGNORECASE,
)


# (category, url host/path fragments, optional window title fragments)
_SITE_RULES: list[tuple[ActivityCategory, tuple[str, ...], tuple[str, ...]]] = [
    ...
    (
        ...
        ("meet.google.com", "zoom.us", "teams.microsoft.com/l/meetup", "whereby.com", "webex.com"),
        ("google meet", "zoom meeting", "zoom -", "webex"),
    ),
]
...
def classify_from_text(text: str | None) -> ActivityCategory | None:
    ...
    if _MEETING_TEXT_SIGNALS.search(haystack):
